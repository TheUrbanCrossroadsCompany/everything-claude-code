...
- 2026-08-06: Cleared the second advisory wave of the day across both lockfiles. `#10` (Dependabot group bump) moved yarn.lock to undici 6.28.0, tar 7.5.22, brace-expansion 1.1.18, fast-uri 3.1.5, and js-yaml 4.3.1 — covering the freshly disclosed undici CRLF/retry/setCookie advisories, the brace-expansion 1.x backport (GHSA-mh99-v99m-4gvg), and fast-uri GHSA-7p8r-x3mc-p8w7. `#12` applied the matching npm-lockfile patch after `npm audit` flagged 3 high findings against its stale pins. Both audits clean again; push-banner probe shows zero open alerts. Confirmed playbook for any advisory wave: patch yarn.lock, then `npm audit fix --package-lock-only` on the root npm lockfile, then revert npm's yarn.lock v1 rewrite.
- 2026-08-06: Merged Dependabot `#6` (fast-uri 3.1.2 -> 3.1.4), closing the two fastify security advisories fixed in 3.1.3/3.1.4 (URI-authority backslash handling). This was the last open Dependabot PR; dependency posture is now fully current across alerts and update PRs. Lesson recorded: the npm audit feed did not flag these advisories while Dependabot did, so dependency passes should check both `yarn npm audit` and the open Dependabot PR list rather than either alone.
